...
router.post('/:sectionId/questions', authenticateToken, requireRole('TEACHER'), async (req, res) => {
    try {
        const { sectionId } = req.params;
        const { userId } = req.user;
        const { type, text, imageUrl, options, correctAnswers, explanation, marks, negativeMarks, difficulty, order, tags } = req.body;
        // Verify section ownership through test
        const section = await prisma.section.findFirst({
            where: {
                id: sectionId,
...
// Update question
router.put('/:sectionId/questions/:questionId', authenticateToken, requireRole('TEACHER'), async (req, res) => {
    try {
...
// Delete question
router.delete('/:sectionId/questions/:questionId', authenticateToken, requireRole('TEACHER'), async (req, res) => {
    try {
        const { questionId } = req.params;
        const { userId } = req.user;
        // Verify question ownership through test
...
router.post('/:sectionId/questions/bulk', authenticateToken, requireRole('TEACHER'), async (req, res) => {
    try {
        const { sectionId } = req.params;
        const { userId } = req.user;
        ...
        const createdQuestions = await Promise.all(questions.map((questionData, index) => prisma.question.create({
            data: {
                sectionId,
