...
router.post('/:testId/start', authenticateToken, async (req, res) => {
    try {
...
router.post('/:testId/answer', authenticateToken, async (req, res) => {
    try {
        const { testId } = req.params;
        const { userId, role } = req.user;
...
function checkAnswer(question, studentAnswer) {
    ...
    if (question.type === 'MCQ') {
        ...
        return correctAnswers.some((answer) => String(answer).toLowerCase().trim() === studentAnswerLower);
    }
    return false;
}
