...
router.get('/:id/analytics', authenticateToken, requireRole('TEACHER'), async (req, res) => {
    try {
        ...
        const analytics = {
            ...
            totalSubmissions: classData.tests.reduce((acc, test) => acc + test.submissions.length, 0),
            averageScore: 0,
            testParticipation: [],
            studentPerformance: []
        };
        ...
        analytics.testParticipation = classData.tests.map((test) => ({
            testId: test.id,
            ...
            averageScore: test.submissions.length > 0
                ? Math.round(test.submissions.reduce((acc, sub) => acc + sub.percentage, 0) / test.submissions.length)
                : 0
        }));
        // Calculate student performance
        analytics.studentPerformance = classData.students.map((enrollment) => {
            const studentSubmissions = classData.tests.flatMap((test) => test.submissions.filter((sub) => sub.studentId === enrollment.studentId));
            const averageScore = studentSubmissions.length > 0
                ? Math.round(studentSubmissions.reduce((acc, sub) => acc + sub.percentage, 0) / studentSubmissions.length)
                : 0;
            return {
