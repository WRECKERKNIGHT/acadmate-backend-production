...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        const { title, description, type, scheduledAt, duration, location, batchIds, participantUids, maxParticipants, isRecurring, recurringPattern } = req.body;
        ...
        const event = await prisma.specialEvent.create({
            data: {
                ...
                participants: participantUids ? {
                    create: participantUids.map((uid) => ({
                        userUid: uid,
                        status: 'INVITED'
                    }))
                } : undefined
...
router.put('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const { title, description, scheduledAt, duration, location, batchIds, participantUids, maxParticipants, status } = req.body;
...
async function checkTeacherBatchAccess(teacherUid, batchIds) {
    const teacher = await prisma.user.findUnique({
...
async function sendEventNotifications(event, action) {
