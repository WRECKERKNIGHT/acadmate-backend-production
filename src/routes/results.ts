...
router.get('/batch-overview', authenticateToken, async (req, res) => {
  try {
    ...
    const batchStats = teacher.batchesTeaching.map(batch => {
      const batchTests = teacher.testsCreated.filter(test => 
        test.submissions.some(sub => sub.student.batchType === batch.type)
      );

      const batchSubmissions = teacher.testsCreated.flatMap(test =>
        test.submissions.filter(sub => sub.student.batchType === batch.type)
      );
...
router.get('/batch/:batchId/subject-analysis', authenticateToken, async (req, res) => {
  try {
    ...
    const subjectResults = Array.from(subjectAnalysis.values()).map(subject => {
      const allScores = subject.tests.flatMap(test => 
        tests.find(t => t.id === test.id)?.submissions.map(sub => sub.percentage) || []
      );
...
router.get('/comparative-analysis', authenticateToken, async (req, res) => {
  try {
    ...
    const batchComparison = await Promise.all(
