...
function generateUID(name: string, batchCode: string, isTeacher: boolean = false): string {
...
function generatePassword(name: string, batchCode: string): string {
...
export async function seedAcadmateData() {
  ...
  try {
    ...
    const batches: { type: BatchType; code: string; ageRange: [number, number] }[] = [
